      ...
      >
        ...
        <div className="px-6 py-4 bg-white flex items-center justify-between border-b border-line">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="font-bold text-sm text-ink-950 truncate">{doc.name}</h3>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button onClick={handleClose} className="p-1.5 rounded text-ink-400 hover:text-ink-950 hover:bg-paper-100 transition-colors" title="Kapat">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
            ...
            <div className="border border-line rounded-xl overflow-hidden">
              ...
              <div className="p-6 space-y-4 bg-paper-50">
                <div>
                  ...
                  <p className="font-bold text-ink-950 text-sm mt-1">{doc.supplier || doc.client}</p>
                </div>
                ...
                <div className="flex items-end gap-6 pt-2 border-t border-line">
                  <div>
                    ...
                    <p className="font-mono text-3xl font-semibold text-ink-950 tracking-tight mt-1">{doc.amount}</p>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-3 pt-2 border-t border-line">
                  <div>
                    ...
                    <p className="font-mono font-semibold text-ink-950 text-sm mt-0.5">{doc.vatAmount || '-'}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-mono uppercase tracking-[0.12em] text-ink-400">Oran</p>
                    <p className="font-mono font-semibold text-ink-950 text-sm mt-0.5">{doc.vatRate || '-'}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-mono uppercase tracking-[0.12em] text-ink-400">Dosya</p>
                    <p className="font-mono font-semibold text-ink-950 text-sm mt-0.5">{doc.fileSize || '-'}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl border border-line bg-paper-50 space-y-2.5">
              <div className="flex items-start gap-2.5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[13px] font-semibold text-ink-950 mt-0.5 leading-snug">{doc.assignedAccount || 'Tekdüzen eşlemesi bekleniyor'}</p>
                </div>
              </div>
            </div>
